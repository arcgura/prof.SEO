...
codebook <- read.csv("~/R/codebook.csv")
excode<-codebook$code_for_exclusion
sarcopenia<-read.table("~/R/Sarcopenia.csv", header = TRUE, sep=",", na.strings = c(excode, "NA","","."), as.is = TRUE )
...
sarco.code<-sarcopenia[c(1,11)]
sarco.date<-sarcopenia[c(1,11)]
...
for(i in 1:nrow(sarco.code) ) {
      ...
      code.row.n<-sum(!is.na(code.row[,c(3:ncol(sarco.code))]))
      sarco.code[i,2]<-code.row.n
}

CT.number<-sum(sarco.code[,2])
print(CT.number)
